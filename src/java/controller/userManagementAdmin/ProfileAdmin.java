    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (changePwd != null) {
            try {
                ...
                adao.updateAdminPassword(currentAccount.getId(), validator.hashPassword(newPassword));
        ...
        if (changeInfo != null) {
            try {
                ...
                if (image != null) {
                    String imgPath = adao.getStaffById(currentAccount.getId()).getImage();
                    deleteFile(imgPath);
                } else {
                    image = adao.getStaffById(currentAccount.getId()).getImage();
                }
                ...
                adao.updateInformationAdmin(currentAccount.getId(), image, firstname, lastname, gender, dob, phone, address);
                currentAccount = adao.getStaffById(currentAccount.getId());
                session.setAttribute("staff", currentAccount);
                json.put("success", true);
                response.getWriter().write(json.toString());
